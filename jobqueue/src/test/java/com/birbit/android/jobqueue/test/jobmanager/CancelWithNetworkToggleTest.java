...
public class CancelWithNetworkToggleTest extends JobManagerTestBase {
    ...
    public void testCancelWithoutNetworkPersistent(boolean async, TagConstraint constraint)
        ...
        jobManager.addJob(new DummyJob(new Params(1).requireNetwork()
                                                    .groupBy("group")
                                                    .addTags("tag")));
        jobManager.addJob(new DummyJob(new Params(2).requireNetwork()
                                                    .groupBy("group")
                                                    .addTags("tag")));
        jobManager.addJob(new DummyJob(new Params(3).requireNetwork()
                                                    .groupBy("group")
                                                    .addTags("tag")));
        final CancelResult[] result = new CancelResult[1];
        if (async) {
        ...
        jobManager.addJob(new PersistentDummyJob(new Params(3).requireNetwork()
                                                              .groupBy("group")
                                                              .addTags("tag"), latchCounter - 1));
        networkUtil.setNetworkStatus(NetworkUtil.METERED, true);
        assertThat("new job should run w/o any issues", runLatch.await(2, TimeUnit.SECONDS), is(true));
    }
