...
public class NetworkJobTest extends JobManagerTestBase {
    ...
    public void testNetworkJob() throws Exception {
        ...
        MatcherAssert.assertThat("count should be correct if there are network and non-network jobs w/o network", jobManager.count(), equalTo(2));
        dummyNetworkUtil.setNetworkStatus(NetworkUtil.METERED);
        MatcherAssert.assertThat("count should be correct if there is network and non-network jobs w/o network", jobManager.count(), equalTo(2));
        dummyNetworkUtil.setNetworkStatus(NetworkUtil.DISCONNECTED);
        final CountDownLatch noNetworkLatch = new CountDownLatch(1);
        ...
        MatcherAssert.assertThat("no network jobs should be executed even if there is no network", jobManager.count(), equalTo(1));
        ...
        if (unmetered) {
            ...
            MatcherAssert.assertThat(networkLatch.getCount(), is(1L));
